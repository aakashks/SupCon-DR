...
class InfoNCELoss(nn.Module):
    def __init__(self, temperature=0.1):
        """
        Initializes the InfoNCE loss function.

        Parameters:
        temperature (float): A temperature scaling factor to control the separation of the logits.
        """
    ...
    def forward(self, embeddings, labels):
        """
        Compute the InfoNCE loss between embeddings.

        Parameters:
        embeddings (torch.Tensor): The embeddings from the model, shape (batch_size * 2, embedding_size),
                                   typically contains both anchor and positive/negative samples.
        labels (torch.Tensor): Labels indicating which samples are positives to each anchor. For self-supervised
                               tasks, this often comes from the construction of the batch where labels are the
                               indices themselves or augmented versions of the same image.
        ...
        # Create labels for the positive samples within the batch
        labels = torch.arange(batch_size, device=device)
        labels = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()

        # Compute log-softmax over the similarities
        exp_similarities = torch.exp(similarity_matrix)
        log_prob = similarity_matrix - torch.log(exp_similarities.sum(1, keepdim=True))

        # Compute the loss as negative log likelihood of positive samples
        nce_loss = -torch.sum(log_prob * labels) / batch_size
